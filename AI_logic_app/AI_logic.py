...
class GenAI:
    ...
    def extract_code(main_url, main_content, query):
        ...
        if response.status_code == 200:
            ...
            GenAI.split_results(main_content, query, code_blocks, main_url)
        else:
            pass
    
    def split_results(main_content, query, code_blocks, main_url):
        global content_words
        content_words = main_content.split()
        content_words = content_words[:100]
        content_words = ' '.join(content_words)
        GenAI.print_results(main_content, query, code_blocks, main_url,content_words)

    def print_results(main_content, query, code_blocks, main_url, content_words):
...
def share_main_content(content_answer):
    my_content_answer = content_answer
